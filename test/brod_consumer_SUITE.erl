...
t_consumer_connection_restart(Config) ->
  ...
  Parent = self(),
  meck:new(brod_client, [passthrough, no_passthrough_cover, no_history]),
  meck:expect(brod_client, get_leader_connection,
              fun(Pid, T, P) ->
                R = meck:passthrough([Pid, T, P]),
                Parent ! {leader_connection, R},
                R
              end),
  ...
  RecvConn =
    fun() ->
        ?WAIT({leader_connection, Result},
              begin {ok, Pid} = Result, Pid end)
    end,
  ConnPid = RecvConn(),
...
t_consumer_connection_restart_2(Config) ->
  ...
  {ok, ConsumerPid} =
    brod_consumer:start_link(?HOSTS, Topic, Partition, ConsumerCfg),
  ProduceFun =
    fun(I) ->
      Value = Key= integer_to_binary(I),
      {ok, Offset} =
  ...
  meck:expect(kpro, connect_partition_leader,
              fun(Endpoints, ConnCfg, T, P) ->
                R = meck:passthrough([Endpoints, ConnCfg, T, P]),
                Parent ! {leader_connection, R},
                R
              end),
  ok = brod_consumer:subscribe(ConsumerPid, self(),
